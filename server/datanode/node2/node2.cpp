...
std::unique_ptr<raichu::server::dbservice> node2dbservice;
                                                           ...
                                                           KVResponse *response)
{
  ...
  if (node2dbservice->commitRequest())
  {
    response->set_message(node2dbservice->commitTransactions(raichu::server::dbservice::Op::READ, key));
    return Status::OK;
  }
                                                          ...
                                                          KVResponse *response)
{
  ...
  if (node2dbservice->commitRequest())
  {
    response->set_message(node2dbservice->commitTransactions(raichu::server::dbservice::Op::PUT, key, value));
                                                             ...
                                                             KVResponse *response)
{
  ...
  if (node2dbservice->commitRequest())
  {
    response->set_message(node2dbservice->commitTransactions(raichu::server::dbservice::Op::DELETE, key));
...
int main(int argc, char **argv)
{
  ...
  node2dbservice.reset(new raichu::server::dbservice(3));
