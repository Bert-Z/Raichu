...
namespace utils
{
    ...
    {
        if (perms == raichu::server::zk::zoo_perm_all)
        {
            return "all";
        }

        std::string ret;
        if (perms & raichu::server::zk::zoo_perm_create)
        {
            ret.append("c");
        }
        if (perms & raichu::server::zk::zoo_perm_read)
        {
            ret.append("r");
        }
        if (perms & raichu::server::zk::zoo_perm_delete)
        {
            ret.append("d");
        }
        if (perms & raichu::server::zk::zoo_perm_write)
        {
            ret.append("w");
        }
        if (perms & raichu::server::zk::zoo_perm_admin)
        {
            ret.append("a");
        }
        return ret;
    ...
    static int32_t perms_string_to_int(const std::string &perm_str)
    {
        if (perm_str == "all")
        {
            return raichu::server::zk::zoo_perm_all;
        }
        ...
        for (auto c : perm_str)
        {
            if (c == 'c')
            {
                perms |= raichu::server::zk::zoo_perm_create;
            }
            else if (c == 'r')
            {
                perms |= raichu::server::zk::zoo_perm_read;
            }
            else if (c == 'd')
            {
                perms |= raichu::server::zk::zoo_perm_delete;
            }
            else if (c == 'w')
            {
                perms |= raichu::server::zk::zoo_perm_write;
            }
            else if (c == 'a')
            {
                perms |= raichu::server::zk::zoo_perm_admin;
            }
        }
        return perms;
    }
...
int main()
{
    ...
    do
    {
        raichu::server::zk::zoo_rc ret = zk.connect(urls);
        if (ret != raichu::server::zk::z_ok)
        {
            printf("try connect zk server failed, code[%d][%s]\n",
                   ret, raichu::server::zk::zk_cpp::error_string(ret));
            break;
        }
        ...
        while (std::cin >> cmd)
        {
            if (cmd == "create")
            {
                ...
                raichu::server::zk::zoo_rc ret = raichu::server::zk::z_ok;
                if (flag == 0)
                {
                ...
                printf("create path[%s] flag[%d] ret[%d][%s], rpath[%s]\n",
                       path.c_str(), flag, ret, raichu::server::zk::zk_cpp::error_string(ret), rpath.c_str());
            }
            else if (cmd == "get")
            {
                std::string path;
                ...
                raichu::server::zk::zoo_rc ret = zk.get_node(path.c_str(), value, nullptr, true);
            ...
            else if (cmd == "set")
            {
                ...
                raichu::server::zk::zoo_rc ret = zk.set_node(path.c_str(), value, -1);
            ...
            else if (cmd == "exist")
            {
                ...
                raichu::server::zk::zoo_rc ret = zk.exists_node(path.c_str(), nullptr, true);
                printf("try_check path[%s] exist[%d], ret[%d][%s]\n",
                       path.c_str(), ret == raichu::server::zk::z_ok, ret, raichu::server::zk::zk_cpp::error_string(ret));
            }
            else if (cmd == "ls")
            {
                std::string path;
                ...
                raichu::server::zk::zoo_rc ret = zk.get_children(path.c_str(), children, true);
            ...
            else if (cmd == "delete")
            {
                ...
                raichu::server::zk::zoo_rc ret = zk.delete_node(path.c_str(), -1);
                printf("try delete path[%s], ret[%d][%s]\n",
                       path.c_str(), ret, raichu::server::zk::zk_cpp::error_string(ret));
            }
            else if (cmd == "setacl")
            {
                ...
                if (scheme == "world")
                {
                    ...
                    auto ac = raichu::server::zk::zk_cpp::create_world_acl(perms);
                    acl.push_back(ac);
                }
                else if (scheme == "auth")
                {
                    ...
                    auto ac = raichu::server::zk::zk_cpp::create_auth_acl(perms);
                    acl.push_back(ac);
                }
                else if (scheme == "digest")
                {
                    ...
                    auto ac = raichu::server::zk::zk_cpp::create_digest_acl(perms, user_name, passwd);
                    acl.push_back(ac);
                }
                else if (scheme == "ip")
                {
                    ...
                    const std::string &perm_str = splits[2];
                    perms = utils::perms_string_to_int(perm_str);
                    auto ac = raichu::server::zk::zk_cpp::create_ip_acl(perms, id);
                    acl.push_back(ac);
                }
                else
                {
            ...
            else if (cmd == "getacl")
            {
                ...
                std::vector<raichu::server::zk::zoo_acl_t> acl;
                raichu::server::zk::zoo_rc rt = zk.get_acl(path.c_str(), acl);
                printf("get acl of path[%s], ret[%d][%s], acl_count[%d]\n",
                       path.c_str(), ret, raichu::server::zk::zk_cpp::error_string(ret), (int32_t)acl.size());
            ...
            else if (cmd == "addauth")
            {
                ...
                raichu::server::zk::zoo_rc rt = zk.add_auth(user_name, user_passwd);
            ...
            else if (cmd == "watch_data")
            {
                ...
                raichu::server::zk::zoo_rc ret = zk.watch_data_change(path.c_str(), data_change_event, &value);
            ...
            else if (cmd == "watch_child")
            {
                ...
                raichu::server::zk::zoo_rc ret = zk.watch_children_event(path.c_str(), child_change_events, &children);
